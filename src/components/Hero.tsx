...
const DeadlineBanner = () => {
  ...
  useEffect(() => {
    // Set the deadline for Dia de las Madres orders.
    // Time is set for Tequila, Jalisco, Mexico (America/Mexico_City, which is UTC-6 during winter).
    const deadline = new Date("2026-05-09T18:00:00-06:00");
    const now = new Date();
  ...
  return (
    <div className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/20 px-4 py-1.5 text-sm font-semibold text-white backdrop-blur-md">
      ...
      ¡No te quedes fuera! Pedidos hasta el 9 de mayo a las 6:00 PM.
    </div>
  );
};

export function Hero() {
  const heroImage = PlaceHolderImages.find((img) => img.id === "hero-mayo");
          ...
          <div className="relative flex min-h-[550px] flex-col items-center justify-center p-8 text-center">
            ...
            <div className="relative z-10 max-w-2xl space-y-6">
              ...
              <h1 className="text-4xl font-extrabold leading-tight tracking-tight text-white md:text-7xl">
                Sorprende este <span className="text-primary">10 de Mayo</span>{" "}
                💖
              </h1>
              <p className="mx-auto max-w-lg font-medium text-white/90 md:text-xl">
                No solo envias flores, envias gratitud y amor. Arreglos
                exclusivos para celebrar a mama como se merece.
              </p>
